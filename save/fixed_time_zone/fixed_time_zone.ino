...
void loop() {
	...
	if ( !(epoch % 60) || ( epoch < 100 ) ) {										// waits for the timer to get 60 secounds
		...
		if (!iReceiveCNT) {															// waiting for the server to response success
			...
			Serial.print(chHour);
			if(chHour != 1073732304){
				chHourlite = chHour + 2;
				if (chHourlite == 24){
					sprintf(strTime,"00:%02i:%02i", chMinute, chSeconds) ;    			// print the hour, minute and second:
					display.drawStr(0, 35, strTime );									// prints the time ( updated)
				}else if(chHourlite == 25){
					sprintf(strTime,"01:%02i:%02i", chMinute, chSeconds) ;    			// print the hour, minute and second:
					display.drawStr(0, 35, strTime );									// prints the time ( updated)
				}else{
	...
	if(chHour != 1073732304){
		...
		Serial.println("The UTC time is " + (String)strTime );
		display.setFont(u8g2_font_profont11_tr);										// small font size
		display.drawStr(0, 8, "Zeit Deutschland:" ) ;									// emblem
		//display.drawStr(0, 8, "tesotec ATE solutions" ) ;								// emblem
		display.drawStr(0, 63, "|" );													// begin line of the secounds bar
